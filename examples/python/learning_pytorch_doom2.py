...
def run(game, agent, actions, num_epochs, frame_repeat, steps_per_epoch=2000):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % num_epochs/5 == 0 and epoch > 0:
            skill_value +=1
            game.set_doom_skill(skill_value)
        ...
        for idx in trange(steps_per_epoch, leave=False, ascii=True, dynamic_ncols=True):
            state = preprocess(game.get_state().screen_buffer)
            ...
            nn.utils.clip_grad_norm_(agent.q_net.parameters(), max_norm=1.0)
...
class DuelQNet(nn.Module):
    ...
    def __init__(self, available_actions_count):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(128, 16, kernel_size=12, stride=6, bias=False),
            nn.BatchNorm2d(16),
            nn.ReLU(),
        )
